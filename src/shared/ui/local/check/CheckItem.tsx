import "./style.scss";
import { MarkView, MarkPosition, MarkState } from "./types";
...
const getViewItem = (rawItem: MarkView): MarkView => {
  return {
    ...rawItem,
    position: rawItem.position ?? MarkPosition.single,
  };
};

const getChild = (state: MarkState | undefined, dict: any, position: MarkPosition | undefined) => {
  let child = dict.empty;
  ...
  if(state === MarkState.done) {
    child = dict.done[position];
  }

  if(state === MarkState.planned) {
    child = dict.planned[position];
  }
...
export const CheckItem = ({ item, onCLick }: CheckItemProps) => {
  ...
  const child = getChild(state, dict, position);
